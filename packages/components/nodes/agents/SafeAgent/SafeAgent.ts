...
/**
 * Guardrail helper:
 * - prompt must include {input}
 * - model returns JSON: {"action":"allow|block","text":"...","reason":"..."}
 */
const applyGuardrail = async (
    guardrailPrompt: string | undefined,
    guardrailModel: BaseChatModel | undefined,
    rawInput: string
): Promise<{ action: 'allow' | 'block'; text: string; reason?: string }> => {
    // If nothing configured, passthrough
    if (!guardrailPrompt || !guardrailModel) {
        return { action: 'allow', text: rawInput }
    }

    try {
        const prompt = PromptTemplate.fromTemplate(transformBracesWithColon(guardrailPrompt))
        const chain = RunnableSequence.from([prompt, guardrailModel, new StringOutputParser()])
        const out = await chain.invoke({ input: rawInput })

        // Hard-require JSON; attempt minimal recovery if model adds extra text
        let jsonText = out
        ...
        const action: 'allow' | 'block' = parsed?.action === 'block' ? 'block' : 'allow'
        const text = typeof parsed?.text === 'string' ? parsed.text : rawInput
        const reason = typeof parsed?.reason === 'string' ? parsed.reason : undefined

        return { action, text, reason }
    } catch (e) {
        console.error('Error in guardrail:', e)
        // Fail-open by default
        return { action: 'allow', text: rawInput }
    }
}

class ConversationalRetrievalSafeToolAgent_Agents implements INode {
    label: string
    name: string
    author: string
    version: number
    ...
    constructor(fields?: { sessionId?: string }) {
        ...
        this.inputs = [
            ...
            // ✅ NEW: Guardrail Prompt + Guardrail Model
            {
                ...
                additionalParams: true,
                optional: true,
                default: `You are an input guardrail for a chat agent.
Return ONLY valid minified JSON with keys: action, text, reason.
- action: "allow" or "block"
- text: if allow, return the (optionally sanitised) input; if block, can be empty
- reason: short reason
...
Input: {input}`
    ...
    async run(nodeData: INodeData, input: string, options: ICommonObject): Promise<string | ICommonObject> {
        ...
        // 2) ✅ Guardrail (NEW) — after moderation, before agent
        const guardrailPrompt = nodeData.inputs?.guardrailPrompt as string | undefined
        const guardrailModel = nodeData.inputs?.guardrailModel as BaseChatModel | undefined

        const gr = await applyGuardrail(guardrailPrompt, guardrailModel, input)

        if (gr.action === 'block') {
            const msg = gr.reason || 'Blocked by guardrail'
            await new Promise((resolve) => setTimeout(resolve, 500))
            if (shouldStreamResponse) {
                streamResponse(sseStreamer, chatId, msg)
            }
            return formatResponse(msg)
        }

        // If allow, use sanitised input
        input = gr.text
...
module.exports = {
    nodeClass: ConversationalRetrievalSafeToolAgent_Agents
}
